...
function buildBuildCard(build,idx){
  ...
  var equipHtml=buildBuildEquipPreview(build._equipment||build.equipment||{});
  ...
  var skillHtml=buildBuildSkillPreview(build._skillIcons||build.equip_skills||[]);
...
function buildBuildSkillPreview(skills){
  ...
  for(var i=0;i<skills.length;i++){
    ...
    var iconId=typeof sk==="number"?sk:(sk.icon||0);

    var url="https://cloudstorage.d2core.com/data_img/d4/skill/"+iconId+".png";

    var name=typeof sk==="object"?(sk.name||""):"";

    var rank=(typeof sk==="object"&&sk.rank>1)?(" x"+sk.rank):"";
...
function openBuildDetail(buildId){
  ...
  if(!build){console.warn('[openBuildDetail] build not found:',buildId);return;}
  ...
  if(simTab){
    ...
    console.warn('[openBuildDetail] simulator tab not found');
  }
}
function showBuildDetail(build,detail){
  ...
  var eq=(detail&&detail.equipment)||build._equipment||{};
  ...
  var skills=(detail&&detail.skillIcons)||build._skillIcons||build.equip_skills||[];
  ...
  var iconId=typeof s==="number"?s:(s.icon||0);
  var url="https://cloudstorage.d2core.com/data_img/d4/skill/"+iconId+".png";
  var sObj=typeof s==="object"?s:{};
  var mods=(sObj.mods||[]).join(" / ");
    html+='<div class="bdm-sk">'+
      ...
      '<div class="bdm-sn">'+escHtml(sObj.name||"")+(sObj.rank>1?(" x"+sObj.rank):"")+"</div>"+
